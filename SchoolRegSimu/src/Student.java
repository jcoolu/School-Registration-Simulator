...
public class Student {
	...
	public Student(String studID, String fName, String lName, String street, String cit, String st, String zipcode,
			String num, String maj, String min, String grade, String image, String numOfCourses, ArrayList<Course> courses, String cred) {
		id = studID;
		firstName = fName;
		lastName = lName;
		address = street;
	...
	public void removeCourseCode(String code) {
		for(int i = 0; i < schedule.size(); i++) {
			if(code.equalsIgnoreCase(schedule.get(i).getCourseCode())) {
				schedule.remove(i);
				int size = Integer.parseInt(this.getNumberOfCourses());
				size--;
				this.setNumberOfCourses("" + size);
			}
		}
	}
...
/*	*//**
	...
	/**
	 * Returns whether course in Students schedule is already added
	 * to their schedule. Returns true if course is already added. 
	 * 
	 * @param course String course name
	 * @return true or false
	 */
	public boolean isAlreadyAdded(String course) {
		boolean result = true;
		for (int i = 0; i < schedule.size(); i++) {
			if (course.trim().equals(schedule.get(i).getCourseName().trim())) {
				result = true;
				break;
			}
			result = false;
	...
	public String toString() {
		return id + "," + firstName + "," + lastName + ","
				...
				+ numberOfCourses + "," 
				+ schedule.toString().replace("[", "").replace("]", "").trim() + ","
				+ credits;
	}
